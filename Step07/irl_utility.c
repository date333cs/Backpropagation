...
unsigned char *alloc_1d_uchar(int n){
  ...
  new = (unsigned char *) malloc ((unsigned) (n * sizeof (unsigned char)));
  if (new == NULL) {
    printf("ALLOC_1D_UCHAR: Couldn't allocate array of uchars\n");
    return (NULL);
  }
  return (new);
}
...
/*** Allocate 2d array of char`s ***/
unsigned char **alloc_2d_uchar(int m, int n){

  int i;
  unsigned char **new;

  new = (unsigned char **) malloc ((unsigned) (m * sizeof (unsigned char *)));
  if (new == NULL) {
    printf("ALLOC_2D_CHAR: Couldn't allocate array of uchar ptrs\n");
    return (NULL);
  }

  for (i = 0; i < m; i++) {
    new[i] = alloc_1d_uchar(n);
  }
  return (new);
}
